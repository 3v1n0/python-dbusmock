...
class TestSystemd(dbusmock.DBusTestCase):
    ...
    def _test_base(self, bus, template):
        dummy_service = 'dummy-dbusmock.service'

        (self.p_mock, obj_systemd) = self.spawn_server_template(template, {},
                                                                subprocess.PIPE)
    ...
    def test_user(self):
        self._test_base(self.session_bus, 'systemd-user')

    def test_system(self):
        self._test_base(self.system_bus, 'systemd-system')
